 ...
 */
  ...
  replaceOrSuppressMost_(kHandler.visual, (event: HandlerNS.Event): HandlerResult => {
    ...
    if (!currentPrefix && !currentKeys && (key === "y" || key === "d" || key.length === 1 && key >= "1" && key <= "5")
        && getSelectionText(1, curSelection)) {
      retainSelection = 1
